...
export function CareerDashboard({ initialData }: { initialData: CareerDashboardData }) {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      setData((current) => ({ ...current, profile: profile ? { id: profile.id, name: profile.name, headline: profile.headline, about: profile.about, location: profile.location, linkedinUrl: profile.linkedinUrl, skills: Array.isArray(profile.skills) ? profile.skills : [], targetTitles: Array.isArray(profile.targetTitles) ? profile.targetTitles : [] } : null, matches: (payload.matches ?? []).map((match: Record<string, unknown>) => { const job = match.job as Record<string, unknown>; return { id: String(job.id ?? match.id), title: String(job.title), company: (job.company as string | null) ?? null, location: (job.location as string | null) ?? null, url: String(job.url), score: Number(match.score), label: String(match.label), matchedSkills: Array.isArray(match.matchedSkills) ? match.matchedSkills as string[] : [], skillGaps: Array.isArray(match.skillGaps) ? match.skillGaps as string[] : [], rationale: String(match.rationale) }; }), radar: payload.radar ?? [], audit: current.audit }));
  ...
  return <main className="admin-shell"><div className="admin-container">
    ...
    <section className="admin-panel"><div className="admin-toolbar"><button className="admin-button" disabled={busy} onClick={() => void loadDashboard()}>Carregar dados</button><button className="admin-button active" disabled={busy} onClick={() => void scan()}>Buscar vagas agora</button><button className="admin-button" disabled={busy} onClick={() => void audit()}>Auditar LinkedIn</button><button className="admin-button" onClick={() => { void fetch("/api/auth/logout", { method: "POST" }).then(() => window.location.assign("/career")); }}>Sair</button></div></section>
